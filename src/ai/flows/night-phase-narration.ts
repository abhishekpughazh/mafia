...
const PromptOutputSchema = z.object({
  narration: z.string().describe('The suspenseful night phase narration text.')
});
...
{{#if currentDay}} It is the start of Night {{currentDay}}.
{{/if}}
...
const nightPhaseNarrationFlow = ai.defineFlow(
  ...
  async input => {
    const {output} = await nightPhaseNarrationPrompt(input || {});
